	...
	public function addOrder_post(){
        ...
        if (!$method == 'POST') {
            ...
            if($isOrderAdded['response']){
				...
				$this->log_m->Log($this->input->post('createdBy'), 'Order','A order is created successfully.');
    ...
    public function dispatchItem_post(){
        ...
        if (!$method == 'POST') {
            ...
            if($checkStock){
                ...
                $response = ['status' => 200, 'message' =>'error', 'description' =>'Product stock is '.round($product->stock,2).' Kg. You can not dispatched more than that.'];
            }
            $this->response($response, REST_Controller::HTTP_OK);
            exit();
        }
